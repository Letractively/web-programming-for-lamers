...
webfxMenuDefaultUseAutoPosition=nn6?false:true;webfxMenuImagePath="";
webfxMenuUseHover=opera?true:false;webfxMenuHideTime=500;
webfxMenuShowTime=200;
var webFXMenuHandler={idCounter:1,idPrefix:"webfx-menu-object-",all:{},getId:function(){return this.idPrefix+this.idCounter++},overMenuItem:function(oItem){if(this.showTimeout!=null)window.clearTimeout(this.showTimeout);

if(this.hideTimeout!=null)window.clearTimeout(this.hideTimeout);
var jCFG_sitem=this.all[oItem.id];
if(webfxMenuShowTime<=0)this._over(jCFG_sitem);else this.showTimeout=window.setTimeout("webFXMenuHandler._over(webFXMenuHandler.all['"+jCFG_sitem.id+"'])",webfxMenuShowTime)},outMenuItem:function(oItem){if(this.showTimeout!=null)window.clearTimeout(this.showTimeout);
if(this.hideTimeout!=null)window.clearTimeout(this.hideTimeout);
var jCFG_sitem=this.all[oItem.id];if(webfxMenuHideTime<=0)this._out(jCFG_sitem);else this.hideTimeout=window.setTimeout("webFXMenuHandler._out(webFXMenuHandler.all['"+jCFG_sitem.id+"'])",webfxMenuHideTime)},blurMenu:function(oMenuItem){window.setTimeout("webFXMenuHandler.all[\"" + oMenuItem.id + "\"].subMenu.hide();",webfxMenuHideTime)},_over:function(jCFG_sitem){top.status=jCFG_sitem.toolTip==''?jCFG_sitem.text:jCFG_sitem.toolTip;if(jCFG_sitem.subMenu){jCFG_sitem.parentMenu.hideAllSubs();jCFG_sitem.subMenu.show()}else jCFG_sitem.parentMenu.hideAllSubs()},_out:function(jCFG_sitem){var root=jCFG_sitem;var m;if(root instanceof WebFXMenuButton)m=root.subMenu;else{m=jCFG_sitem.parentMenu;while(m.parentMenu!=null&&!(m.parentMenu instanceof WebFXMenuBar))m=m.parentMenu}if(m!=null)m.hide()},hideMenu:function(menu){if(this.showTimeout!=null)window.clearTimeout(this.showTimeout);
if(this.hideTimeout!=null)window.clearTimeout(this.hideTimeout);this.hideTimeout=window.setTimeout("webFXMenuHandler.all['"+menu.id+"'].hide()",webfxMenuHideTime)},showMenu:function(menu,src,dir){if(this.showTimeout!=null)window.clearTimeout(this.showTimeout);
if(this.hideTimeout!=null)window.clearTimeout(this.hideTimeout);
if(arguments.length<3)dir="vertical";menu.show(src,dir)}};
...
function WebFXMenu(){
	...
	webFXMenuHandler.all[this.id]=this}WebFXMenu.prototype.width=webfxMenuDefaultWidth;
